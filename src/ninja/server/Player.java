...
public class Player extends Char
{
            ...
            if (item != null) {
                if (indexUI != -1) {
                    ...
                    if (item.isItemBodyEffect()) {
                        Char.removeEffect(_char, Effect.itemEffectId(item.template.id));
                    }
            ...
            if (percen < 40) {
                ...
                if (isCoin) {
                    ...
                    if (_char.user.player.yen >= GameScr.coinUpCrystals[id]) {
                        _char.user.player.upCoinLock(-GameScr.coinUpCrystals[id], (byte)0);
                    }
                    else {
                        final int coin = GameScr.coinUpCrystals[id] - _char.user.player.yen;
                        _char.user.player.upCoinLock(-_char.user.player.yen, (byte)0);
    ...
    protected static void UpGrade(final Char _char, final Item item, final Item[] items, final boolean isGold) {
        ...
        if (coins <= _char.user.player.yen) {
            _char.user.player.upCoinLock(-coins, (byte)0);
        }
        else {
