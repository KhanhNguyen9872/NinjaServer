...
public class GameCanvas
{
    ...
    protected static void getImgEffectAuto(final Char _char, final short id) {
        Message msg = null;
        try {
            final byte[] ab = NinjaUtil.getFile("cache/Effect/x" + _char.user.session.zoomLevel + "/ImgEffect/ImgEffect " + id + ".png");
            if (ab != null) {
                msg = new Message((byte)122);
    ...
    protected static void getDataEffectAuto(final Char _char, final short id) {
        Message msg = null;
        try {
            final byte[] ab = NinjaUtil.getFile("cache/Effect/x" + _char.user.session.zoomLevel + "/DataEffectAuto/" + id);
            if (ab != null) {
                msg = new Message((byte)122);
                msg.writer().writeByte(2);
                msg.writer().writeByte(id);
