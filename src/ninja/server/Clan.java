...
public class Clan
{
            ...
            synchronized (Clan.LOCK) {
                if (Clan.clan_name.get(clanName) != null) {
                    ...
                    try {
                        ...
                        try {
                            _char.user.player.upGold(-200000L, (byte)0);
    ...
    protected void flush() {
        try {
            ...
            sql = sql + ",`members`='" + jarrMem.toJSONString() + "',`numMem`=" + this.numMember + ",`maxMem`=" + this.maxMember + ",`items`='" + jarrItem.toJSONString() + "',`debt`=" + this.debt + ",`isDissolution`=" + this.isDissolution + ",`tolltimeWeek`=" + this.tolltimeWeek + "";
            final MySQL mySQL = new MySQL(1);
            try {
