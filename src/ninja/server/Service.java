...
public class Service
{
    ...
    protected static void MELoadALL(final Char _char) {
        Message msg = null;
        try {
            msg = messageSubCommand((byte)(-127));
            ...
            if (CaiTrang.Head(_char.partCaiTrang()) != -1) {
                msg.writer().writeShort(CaiTrang.Head(_char.partCaiTrang()));
            }
            ...
            msg.writer().writeShort(_char.potential[0]);
            msg.writer().writeShort(_char.potential[1]);
            ...
            msg.writer().writeShort(CaiTrang.Body(_char.partCaiTrang()));
            msg.writer().writeShort(CaiTrang.Leg(_char.partCaiTrang()));
    ...
    protected static void PlayerLoadAll(final Char _char, final Char player) {
        ...
        try {
            ...
            if (CaiTrang.Head(player.partCaiTrang()) != -1) {
                msg.writer().writeShort(CaiTrang.Head(player.partCaiTrang()));
            }
            ...
            msg.writer().writeShort(CaiTrang.Body(player.partCaiTrang()));
            msg.writer().writeShort(CaiTrang.Leg(player.partCaiTrang()));
    ...
    protected static void initSelectChar(final User user) {
        ...
        try {
            ...
            for (byte num = 0; num < user.numPlayer; ++num) {
                ...
                if (CaiTrang.Body(_char.partCaiTrang()) != -1) {
                    msg.writer().writeShort(CaiTrang.Body(_char.partCaiTrang()));
                }
                ...
                if (CaiTrang.Leg(_char.partCaiTrang()) != -1) {
                    msg.writer().writeShort(CaiTrang.Leg(_char.partCaiTrang()));
                }
    ...
    protected static void requestLogin(final Session_ME session) throws Exception {
        Message msg = null;
        try {
    ...
    protected static void sendMSG(final Session_ME session, final int b) throws Exception {
        final byte[] ab = NinjaUtil.getFile("cache/msg/-28_" + b);
        final Message msg = new Message(-28);
        msg.writer().write(ab);
        session.sendMessage(msg);
        msg.cleanup();
    }
    
    protected static void send(final Session_ME session, final byte m, final String s) {
        final byte[] ab = NinjaUtil.getFile("cache/msg/" + m + "" + s + "");
        try {
            final Message msg = new Message(m);
            msg.writer().write(ab);
            session.sendMessage(msg);
            msg.cleanup();
        }
    ...
    protected static void send(final Session_ME session, final byte m) {
        final byte[] ab = NinjaUtil.getFile("cache/msg/" + m);
        try {
            final Message msg = new Message(m);
            msg.writer().write(ab);
            session.sendMessage(msg);
    ...
    protected static void requestClanItem(final Char _char, final Clan clan) {
        ...
        try {
            ...
            for (byte i = 0; i < 2; ++i) {
                final ItemTemplate template = GameScr.itemTemplates[Clan_ThanThu.arrIdTrung[i]];
                msg.writer().writeUTF(template.name);
                msg.writer().writeShort(template.iconID);
                msg.writer().writeShort(Clan_ThanThu.arriconIdThanThu[i][0]);
                msg.writer().writeInt(604800000);
                msg.writer().writeByte(4);
                msg.writer().writeUTF(Util.toDateString(Date.from(Instant.ofEpochMilli(System.currentTimeMillis() + 60000L))));
                msg.writer().writeByte(4);
            }
            _char.user.session.sendMessage(msg);
        }
    ...
    protected static void ItemInfo(final Char _char, final Item item, final byte typeUI, final int indexUI) {
        ...
        try {
            ...
            if (item.isTypeBody() || item.isTypeMounts() || item.isTypeNgocKham()) {
                ...
                NinjaUtil.writeByteArray(msg, new byte[0]);
            }
        ...
        try {
            ...
            for (byte i = 0; i < 16; ++i) {
                if (player.ItemBody[i] != null) {
                    ...
                    final String str = _char.user.session.versionNja.replace(".", "");
                    final int numVersion = Integer.parseInt(str);
                    if (numVersion >= 180) {
                        msg.writer().writeShort(-1);
                    }
                }
