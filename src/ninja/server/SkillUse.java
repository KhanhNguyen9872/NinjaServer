                        ...
                        if (arrChar != null) {
                            for (byte k = 0; k < arrChar.length; ++k) {
                                ...
                                if (player != null && (myChar.isTest || (myChar.cTypePk == 4 && player.cTypePk == 5) || (myChar.cTypePk == 5 && player.cTypePk == 4) || (myChar.cTypePk == 1 && player.cTypePk == 1 && !MapServer.notCombat(myChar.tileMap.mapID)) || ((player.cTypePk == 3 || myChar.cTypePk == 3) && !MapServer.notCombat(myChar.tileMap.mapID) && (!player.isNotPK() || _char.isNhanban)) || (player.cTypePk == 4 && _char.cTypePk == 5) || (player.cTypePk == 5 && _char.cTypePk == 4) || (_char.isTest && player.isTest && _char.testCharID == player.charID && _char.charID == player.testCharID) || (_char.KillCharId == player.charID && (!player.isNotPK() || _char.isNhanban)) || player.KillCharId == _char.charID)) {
                                    ...
                                    int newdame = Util.nextInt(dame * 9 / 10, dame) / 10;
                                    if (!_char.isHuman) {
                                        newdame = newdame * _char.percentPow / 100;
                                    }
                                    if (dame > 0 && newdame <= 0) {
                                        newdame = 1;
                                    }
                ...
                if (!mob.injureThenDie && mob.status != 0 && mob.status != 1) {
                    ...
                    if (mob.hp <= 0) {
                        ...
                        if (tileMap.map.isChienTruong()) {
                            ...
                            if (mob.templateId != 0 && mob.levelBoss != 3 && !mob.isBoss) {
                                Item item = null;
                                if (Util.nextInt(100) < 50 && Task.itemTemplateId(myChar, mob) != -1) {
                                    item = new Item(null, Task.itemTemplateId(myChar, mob), 1, -1, true, (byte)0, 0);
                                }
                                else if ((((!tileMap.map.isMonsterMap() && Util.nextInt(100) <= 20) || (tileMap.map.isMonsterMap() && Util.nextInt(100) <= 5)) && Math.abs(myChar.cLevel - mob.level) <= 7) || (tileMap.map.isMapLangCo() && Util.nextInt(100) <= 10)) {
                                ...
                                else if (Util.nextInt(100) < 5 && Math.abs(myChar.cLevel - mob.level) <= 7) {
                                    final int level = (mob.level >= 10 && mob.level < 20) ? 10 : ((mob.level >= 20 && mob.level < 30) ? 20 : ((mob.level >= 30 && mob.level < 40) ? 30 : ((mob.level >= 40 && mob.level < 50) ? 40 : ((mob.level >= 50 && mob.level < 60) ? 60 : ((mob.level >= 60 && mob.level < 70) ? 60 : 1)))));
                                    item = ItemServer.itemWPShop(level, (byte)Util.nextInt(1, 6), (byte)2);
                                }
                                else if (tileMap.map.isMapLangCo() && Util.nextInt(100) <= 50) {
                                    if (Util.nextInt(100) <= 18) {
                                ...
                                if (item != null) {
                                    itemmap = new ItemMap(item, playerId, 65000, mob.x, mob.y);
                                    tileMap.ItemMapADD(itemmap);
                                }
                            }
                            else if (mob.isBoss) {
                                final ArrayList<ItemMap> AitemMap = new ArrayList<ItemMap>();
                                ...
                                if (mob.templateId == 138) {
                                    for (byte j = 0; j < Util.nextInt(5, 15); ++j) {
                                        ...
                                        AitemMap.add(new ItemMap(item2, playerId, 65000, x, y));
                                    }
                                }
                                if (!tileMap.map.isMapLangCo() && mob.templateId != 230) {
                                    if (mob.templateId == 114 || mob.templateId == 115 || mob.templateId == 116 || mob.templateId == 139 || mob.templateId == 201 || mob.templateId == 203 || mob.templateId == 204) {
                                        ...
                                        for (byte k = 0; k < 4 && quantity > 0; quantity -= (tileMap.map.isMonsterMap() ? 2 : 1), ++k) {
                                            for (byte m = 0; m < quantity; ++m) {
                                                ...
                                                AitemMap.add(new ItemMap(item3, playerId, 65000, x2, y2));
                                            }
                                        }
                                        if (tileMap.map.isMonsterMap()) {
                                            for (byte k = 0; k < Util.nextInt(0, 2); ++k) {
                                                ...
                                                AitemMap.add(new ItemMap(item4, playerId, 65000, x3, y3));
                                            }
                                        }
                                    }
                                    if (mob.templateId == 114 || mob.templateId == 116 || mob.templateId == 139 || mob.templateId == 201 || mob.templateId == 203 || mob.templateId == 204) {
                                        if (Util.nextInt(10) >= 5) {
                                            ...
                                            AitemMap.add(new ItemMap(new Item(null, (short)(tileMap.map.isMonsterMap() ? Util.nextInt(558, 563) : Util.nextInt(311, 316)), 1, -1, false, (byte)0, 0), playerId, 65000, x4, y4));
                                        }
                                        if (tileMap.map.isMonsterMap() && Util.nextInt(10) == 0) {
                                            ...
                                            AitemMap.add(new ItemMap(item5, playerId, 65000, x4, y4));
                                        }
                                    }
                                    if (mob.templateId == 114 || mob.templateId == 115 || mob.templateId == 116 || mob.templateId == 139 || mob.templateId == 201 || mob.templateId == 203 || mob.templateId == 204) {
                                        short x4 = mob.x;
                                        ...
                                        AitemMap.add(new ItemMap(new Item(null, (short)340, tileMap.map.isMonsterMap() ? 5 : 1, -1, false, (byte)0, 0), playerId, 65000, x4, y4));
                                    }
                                    if (mob.templateId == 114 || mob.templateId == 115 || mob.templateId == 116 || mob.templateId == 139 || mob.templateId == 201 || mob.templateId == 203 || mob.templateId == 204) {
                                        for (byte j = 0; j < Util.nextInt(4, 5); ++j) {
                                            ...
                                            final Item item2 = ItemServer.itemWPShop(Util.nextInt(1, 8) * 10, (byte)Util.nextInt(1, 6), (byte)1);
                                            if (Util.nextInt(10) <= 3) {
                                                item2.upgradeNext((byte)Util.nextInt(7, 10));
                                            }
                                            AitemMap.add(new ItemMap(item2, playerId, 65000, x, y));
                                        }
                                    }
        ...
        if (skill != null && !_char.isIce && !_char.isWind) {
            switch (skill.template.id) {
                case 4: {
                    ...
                    final ItemMap itemMap = new ItemMap(item, _char.playerId, 65000, mob.x, mob.y);
