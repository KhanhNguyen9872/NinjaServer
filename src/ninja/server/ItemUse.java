...
public class ItemUse
{
    ...
    protected static void ItemUse(final Char _char, final byte indexUI) {
        ...
        if (item != null) {
            if (item.template.level > _char.cLevel) {
            ...
            else if (item.isTypeMounts() && item.saleCoinLock > 0) {
                ...
                if (!_char.isHuman) {
                    ...
                    final Map map = MapServer.getMapServer(138);
                    if (map != null) {
                        final TileMap tileMap = map.getSlotZone(_char);
                        if (tileMap == null) {
                            GameCanvas.startOKDlg(_char.user.session, Text.get(0, 9));
                ...
                if (_char.nClass == 0) {
                    ...
                    Item it = null;
                    if (itemType == 1) {
                        it = ItemServer.itemWPShop(level, _char.nClass, (byte)0);
                    }
                    else {
