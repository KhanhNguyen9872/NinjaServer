...
import ninja.template.MobTemplate;
    ...
    protected static Item itemEventMob(final Char _char, final Mob mob) {
        switch (GameScr.vEvent) {
            ...
            case 2: {
                int percent = 5;
                int dlevel = 5;
                final boolean isLock = _char.getEffId((byte)40) == null;
                if (_char.getEffId((byte)40) != null || _char.getEffId((byte)41) != null) {
                    percent *= 2;
                    dlevel = 10;
                }
                if (_char.cLevel >= 40 && Math.abs(_char.cLevel - mob.level) <= dlevel && Util.nextInt(100) < percent) {
                    return new Item(null, (short)Util.nextInt(607, 610), 1, -1, isLock, (byte)0, 0);
                }
                if (mob.level >= 20 && _char.getEffId((byte)43) != null && Util.nextInt(100) < percent && (_char.getEffId((byte)40) != null || _char.getEffId((byte)41) != null || Math.abs(_char.cLevel - mob.level) <= 10)) {
    ...
    protected static void Lighting(final Char _char, final Npc npc) {
        if (_char.user.player.ItemBagIndexNull() == -1) {
            ...
            if (Util.nextInt(100) < 15) {
                ...
                _char.user.player.upCoinLock(Util.nextInt(1000, 10000), (byte)1);
            }
                ...
                if (mob.templateId == 230) {
                    ...
                    if (indexUI != -1) {
                        ...
                        if (Util.nextInt(100) < 25) {
                            ...
                            if (_char.clan != null) {
                                Char.upPointClan(_char, Util.nextInt(25, 50));
                            }
                        }
